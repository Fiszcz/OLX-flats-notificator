...
export class Iteration {
    ...
    private compareTimes = (firstTime: Time, secondTime: Time): TimeComparison => {
        const difference = (firstTime.hour === 0 ? 24 : firstTime.hour) * 60 + firstTime.minutes - secondTime.hour * 60 - secondTime.minutes;
        if (difference > 0)
            return TimeComparison.FIRST_NEWER;
        else if (difference === 0)
            return TimeComparison.THE_SAME;
        return TimeComparison.SECOND_NEWER;
    };

    public isNewerThanPreviousIteration = (advertisementTime?: Time, advertisement?: string) => {
        if (advertisementTime === undefined || advertisement === undefined)
            return false;
        ...
        if (timesComparisonBetweenPreviousIterationAndAdvertisement === TimeComparison.THE_SAME)
            return this.latestAdvertisementsFromPreviousIteration.findIndex((previousAdvertisement) => previousAdvertisement === advertisement) === -1;
